...
class VanDerPolOscillatorsNetwork(nn.Module):
    """
    Van der Pol Oscillators Network. A network model with
    Van der Pol's coupled oscillators, where each node is a nonlinear system
    defined with the Van der Pol's equation:

    .. math::
        \ddot{x} - \mu (1 - x^2) \dot{x} + x = 0

    The model is defined by the following ordinary
    differential equation:

    .. math::
        \\dot{h} = -\\gamma h - \\epsilon (1-h^2) \\dot{h} + \\tanh(W_{in} x + W_{rec} h + b)
    ...
    def __init__(
        ...
        """Initialize the Van der Pol Oscillator Network model.
