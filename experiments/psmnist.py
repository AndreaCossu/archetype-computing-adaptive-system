...
def test(data_loader, classifier, scaler):
    """Score a fitted classifier on permuted sequential MNIST activations.

    :param data_loader: Loader yielding permuted MNIST batches.
...
for i in range(args.trials):
    ...
    train_loader, valid_loader, test_loader = get_mnist_data(
        args.dataroot, args.batch, args.batch, permuted = True
    )
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"psMNIST_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.esn:
    f = open(os.path.join(args.resultroot, f"psMNIST_log_ESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
