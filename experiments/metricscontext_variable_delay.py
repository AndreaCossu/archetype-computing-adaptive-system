...
def make_sequence_pair(delay, args, rng):
    x1 = np.zeros((delay + 2, 1), dtype=np.float32)
    x2 = np.zeros((delay + 2, 1), dtype=np.float32)
    values = stimulus_values(args)

    sample_1 = int(rng.integers(0, 2))
    sample_2 = 1 - sample_1
    probe = int(rng.integers(0, 2))
    distractor = make_distractor(delay, args, rng)

    x1[0, 0] = values[sample_1]
    x2[0, 0] = values[sample_2]
    x1[1 : delay + 1, 0] = distractor
    x2[1 : delay + 1, 0] = distractor
    x1[delay + 1, 0] = values[probe]
    x2[delay + 1, 0] = values[probe]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Context-distance metric for the variable-delay match-to-sample task"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    with torch.no_grad():
        for delay in delays:
            for _ in range(args.num_samples):
                input_sequence1, input_sequence2 = make_sequence_pair(delay, args, rng)

                h1 = initial_state(args)
                h2 = clone_state(args, h1)
                for t in range(delay + 2):
                    h1 = step_reservoir(reservoir, args, input_sequence1[t], h1)
                    h2 = step_reservoir(reservoir, args, input_sequence2[t], h2)
                    if t == 0:
