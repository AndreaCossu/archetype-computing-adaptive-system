...
parser.add_argument("--n_hid", type=int, default=150)
...
if args.trials > 1:
    ...
    train_loader, valid_loader, test_loader = get_libras_data(args.batch, args.batch, whole_train = True)
else:
    train_loader, valid_loader, test_loader = get_libras_data(args.batch, args.batch)
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"Libras_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.esn:
    f = open(os.path.join(args.resultroot, f"Libras_log_ESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
