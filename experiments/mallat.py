...
def test(data_loader, classifier, scaler):
    """Score a fitted classifier on Mallat reservoir activations.

    :param data_loader: Loader yielding Mallat batches.
...
if args.trials > 1:
    ...
    train_loader, valid_loader, test_loader = get_mallat_data(args.batch, 30, whole_train = True)
else:
    train_loader, valid_loader, test_loader = get_mallat_data(args.batch, 30)
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"Mallat_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.esn:
    f = open(os.path.join(args.resultroot, f"Mallat_log_ESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
