...
def test(data_loader, classifier, scaler):
    """Score a fitted classifier on CIFAR-10 reservoir activations.

    :param data_loader: Loader yielding image batches.
    ...
    for images, labels in tqdm(data_loader):
        ...
        images = images.view(images.shape[0], 3, -1).permute(0, 2, 1)
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"CIFAR-10_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.esn:
    f = open(os.path.join(args.resultroot, f"CIFAR-10_log_ESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
