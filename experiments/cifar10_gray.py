...
def test(data_loader, classifier, scaler):
    """Score a fitted classifier on grayscale CIFAR-10 activations.

    :param data_loader: Loader yielding grayscale image batches.
...
for i in range(args.trials):
    ...
    train_loader, valid_loader, test_loader = get_cifar10_data(
        args.dataroot, args.batch, args.batch, grayscale = True
    )
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"CIFAR-10_gray_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.esn:
    f = open(os.path.join(args.resultroot, f"CIFAR-10_gray_log_ESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
