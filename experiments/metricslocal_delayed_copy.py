...
for ts in Ts:
    for n in range(args.num_samples):
        input_sequence = torch.randn(ts + args.context_length, args.input_size)

        h1 = initial_state()
        h2 = clone_state(h1)
        if args.model == 'deep_reservoir':
            h2 += torch.randn_like(h2) * 1e-6
        else:
            h2 = tuple(component + torch.randn_like(component) * 1e-6 for component in h2)

        for t in range(ts + args.context_length):
            h1 = step_reservoir(input_sequence[t], h1)
            h2 = step_reservoir(input_sequence[t], h2)
