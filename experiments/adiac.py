...
def test(data_loader, classifier, scaler):
    """Score a fitted classifier on reservoir activations.
...
if args.ron:
    ...
    f = open(os.path.join(args.resultroot, f"Adiac_log_VDPON_{args.topology}{args.resultsuffix}.txt"), "a")
elif args.pron:
    ...
    f = open(os.path.join(args.resultroot, f"Adiac_log_EUESN{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
